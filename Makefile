...
test: on-app## Run phpunit test app integration with database
	docker exec -t laravel_app tests/phpunit --no-coverage --testdox -c tests/phpunit.xml

test-library: on-app ## Run phpunit test library only - testdox
	docker exec -t laravel_app tests/phpunit --no-coverage --testdox -c tests/phpunit.lib.xml

test-laravel: on-app ## Measure test coverage
	docker exec -t -e XDEBUG_MODE=coverage laravel_app tests/phpunit -d memory_limit=-1 -c tests/phpunit.xml

clean-test-laravel: ## Clean test coverage results
	rm -rf tests/laravel
...
# 		docker-compose exec laravel_db mysql -pcdmllove $$dbname -e "update crm_member set member_used_2auth=0 where member_id=1;";\
# 	done
#
